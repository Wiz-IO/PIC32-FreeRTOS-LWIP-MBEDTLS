                                 ...
                                 unsigned char *output )
{
    ...
    {
        LOG_I(common, "padding:%d", ctx->padding);
        return( MBEDTLS_ERR_RSA_BAD_INPUT_DATA );
    }
    ...
    {
        LOG_I(common, "f_rng:%x, input:%x, output:%x", f_rng, input, output);
        return( MBEDTLS_ERR_RSA_BAD_INPUT_DATA );
    }
    ...
    {
        LOG_I(common, "ilen + 11:%d, ilen:%d, olen:%d", ilen + 11, ilen, olen);
        return( MBEDTLS_ERR_RSA_BAD_INPUT_DATA );
    }
                                 ...
                                 size_t output_max_len)
{
    ...
    if( ilen < 16 || ilen > sizeof( buf ) )
    {
        LOG_I(common, "ilen:%d, sizeof( buf ):%d, output:%x", ilen, sizeof( buf ));
        return( MBEDTLS_ERR_RSA_BAD_INPUT_DATA );
    }
