 ...
 * 
 *  https://github.com/ARMmbed/mbedtls/blob/master/programs/test/selftest.c
 * 
 */
...
static int run_test_snprintf(void) {
    test_snprintf(0, "xxxxxxxxx", -1);
    test_snprintf(1, "", -1); // FAIL ret 0, buf 'xxxxxxxxx'
    test_snprintf(2, "1", -1);
    test_snprintf(3, "12", -1);
    test_snprintf(4, "123", 3);
    test_snprintf(5, "123", 3);
}
...
void ssl_test(void) {
    ...
    for (test = selftests; test->name != NULL; test++) {
        if (test->function(1) != 0) {
            suites_failed++;
        }
        suites_tested++;
    }
    
    LOG("--- SSL TEST END --- TESTED: %d  FAILED: %d \n", suites_tested, suites_failed);
    while (1);
}
